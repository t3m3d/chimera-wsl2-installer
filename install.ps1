[CmdletBinding()]
param(
    [string]$DistroName = 'Chimera',
    [string]$InstallPath = 'C:\WSL\chimera',
    [string]$DownloadDir = 'C:\WSL\downloads',
    [string]$Username = $env:USERNAME.ToLower(),
    [string]$Hostname = "$env:COMPUTERNAME-chimera".ToLower(),
...
function Say([string]$msg) { Write-Host "[chimera-wsl2] $msg" -ForegroundColor Cyan }
...
Say "discovering latest chimera-linux-x86_64-ROOTFS from $RootfsBase"
$sumsUrl = "$RootfsBase/sha256sums.txt"
try {
    $sums = (Invoke-WebRequest -Uri $sumsUrl -UseBasicParsing -TimeoutSec 30).Content
} catch { Fail "couldn't fetch sha256sums.txt at $sumsUrl : $_" }
...
if (-not $match) { Fail "couldn't find chimera-linux-x86_64-ROOTFS-*-full.tar.gz line in sha256sums.txt" }
$expectedSha256 = $match[1]
$stageDate = $match[2]
$stageFilename = "chimera-linux-x86_64-ROOTFS-$stageDate-full.tar.gz"
...
if ($needDownload) {
    Say "downloading $stageFilename (~115 MB)"
...
Say "writing /etc/wsl.conf, creating user '$Username', apk update + base install"
...
apk add shadow sudo bash nano chimera-bootstrap
...
$tmpScript = Join-Path $env:TEMP "chimera-setup-$([Guid]::NewGuid().ToString('N').Substring(0,8)).sh"
...
& $WSL -d $DistroName -u root -- sh "$wslPath" 2>&1 | Tee-Object -Variable setupOut | Out-Host
...
Say "smoke test"
try {
    & $WSL -d $DistroName -- bash -c 'echo "user=$(whoami) uid=$(id -u)"; echo "apk:"; apk --version 2>&1 | head -1; echo "kernel:"; uname -r' 2>&1 | Out-Host
} catch {
...
Write-Host "  Chimera installed and ready"                -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Green
